...
public class DataBuffer {
    ...
    {
    	System.out.println(" ReadChar() buffer[position] = "+buffer[position] );
    ...
    public char[] ReadChars()
    {
        ...
        charsLength = len;		// added for continue play command, _CardCount = readchars();
...
//        return string((char *)ReadRawBytes(len));
  ...
  //length为想要写入的长度，data_length为数据的长度
    public void WriteBytes( char[] data,int startPos,int length, int data_length)
    {
    ...
    public void WriteRawBytes( char[] data,int startPos,int length, int data_length)
    {
        //if(data==NULL || startPos<0 || length<=0 || startPos>data_length)
    	if(startPos<0 || length<=0 || startPos>data_length)
        {
    ...
    {
		position+=8;
		return ((((((((buffer[position-1] & 0xffL) << 8 | buffer[position-2] & 0xffL) << 8 | buffer[position-3] & 0xffL) << 8 
			| buffer[position-4] & 0xffL) << 8 | buffer[position-5] & 0xffL) << 8 | (buffer[position-6] & 0xffL)) << 8) 
			| (buffer[position-7] & 0xffL)) << 8 | (buffer[position-8] & 0xffL); 
    }
